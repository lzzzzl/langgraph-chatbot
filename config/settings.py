...
class LLMSettings(BaseSettings):
    ...
    openrouter_model: str = Field(
        default="anthropic/claude-3.5-sonnet", description="OpenRouter 模型名称"
    )
...
class RedisSettings(BaseSettings):
    ...
    socket_connect_timeout: int = Field(
        default=5, gt=0, description="连接超时时间（秒）"
    )
...
class DatabaseSettings(BaseSettings):
    ...
    sqlite_path: str = Field(
        default="data/chatbot.db", description="SQLite 数据库文件路径"
    )
...
class CheckpointerSettings(BaseSettings):
    ...
    checkpointer_type: Literal["memory", "sqlite", "postgresql", "redis"] = Field(
        default="sqlite", description="检查点存储类型"
    )

    # SQLite 检查点配置
    sqlite_path: str = Field(
        default="data/checkpoints/checkpoints.db", description="SQLite 检查点文件路径"
    )
...
class VectorStoreSettings(BaseSettings):
    ...
    embedding_provider: Literal["openai", "huggingface"] = Field(
        default="openai", description="Embedding 提供商"
    )
...
class MonitoringSettings(BaseSettings):
    ...
    langsmith_project: str = Field(
        default="langgraph-chatbot", description="LangSmith 项目名"
    )
